...
class WavSoundImageMNIST(data.Dataset):
    """ This loads the sound files as is, as a tensor. Not used as a spectrogram.
    ...
    def __getitem__(self, index):
        ...
        audio_transform = torchaudio.transforms.PadTrim(max_len=5000)
        audio = audio_transform(audio.t()).t()
    ...
    def read_audio_data(self):
        path = os.path.join(self.root, '../audio_mnist')
        ...
        for root, _, fnames in sorted(os.walk(os.path.join(path, 'recordings'))):
            for fname in fnames:
                if fname.split('.')[1] == 'wav':
                    ...
                    audio_tensor = self.read_wav(path_temp)

                    if id < 10:     # Test image
                        test_tensors[cls].append(audio_tensor)
                    else:           # Train image
                        train_tensors[cls].append(audio_tensor)

        self.audio_train = train_tensors
        self.audio_test = test_tensors
